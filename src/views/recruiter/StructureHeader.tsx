...
export default function StructureHeader({ onBackToThreshold }: StructureHeaderProps) {
  return (
    <header className="relative w-full border-b border-cream/10 pb-8 sm:pb-12 pt-6 sm:pt-8 md:pt-12">
      {/* ================= 1. TOP TELEMETRY & NAV SWITCHER ================= */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-6 sm:pb-8 text-xs font-mono text-[#8A847C]">
        {/* Top telemetry breadcrumbs */}
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] sm:text-xs tracking-[0.10em] sm:tracking-[0.18em] uppercase text-cream/70">
          <span className="text-ochre font-semibold shrink-0">STRUCTURE://</span>
          <span className="text-cream shrink-0">THE SHORT READ</span>
          <span className="hidden md:inline-block text-cream/30">//</span>
          <span className="hidden md:inline-block text-cream/50">HIGH-SIGNAL RECRUITER DIGEST</span>
        </div>

        {/* Switcher Controls */}
        <div className="flex items-center gap-2.5 sm:gap-3 text-[10px] sm:text-[11px] tracking-wider uppercase self-start sm:self-auto">
          {onBackToThreshold ? (
            <button
              onClick={onBackToThreshold}
              className="group inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded border border-cream/15 bg-cream/[0.03] text-cream/70 hover:text-cream hover:border-cream/40 transition-colors"
          ...
          <Link
            ...
            className="group inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded border border-ochre/30 bg-ochre/[0.08] text-ochre hover:text-cream hover:bg-ochre/20 hover:border-ochre/60 transition-all"
          >
      ...
      <div className="w-full space-y-2 sm:space-y-3 pt-2 sm:pt-0">
        {/* Architectural Kicker */}
        <div className="font-mono text-[11px] sm:text-xs md:text-sm tracking-[0.08em] sm:tracking-[0.20em] text-ochre uppercase flex flex-wrap items-center gap-x-2 gap-y-1 leading-normal">
          <span className="text-ochre font-semibold shrink-0">SYS://</span>
          <span className="text-cream/90 font-medium">INFORMATICS &amp; SOFTWARE ENGINEERING</span>
          <span className="hidden sm:inline-block text-cream/30">//</span>
          <span className="hidden sm:inline-block text-cream/50">RESEARCH &amp; PRODUCTION SYSTEMS</span>
        </div>

        {/* Full-width Horizontal Name (Guaranteed Single-Line on Desktop) */}
        <h1 className="font-mono text-2xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold tracking-[0.02em] sm:tracking-[0.04em] text-cream uppercase leading-[1.15] sm:leading-tight sm:whitespace-nowrap break-words">
          BAGJA ISKANDAR JAMIL
        </h1>
      </div>

      {/* ================= 3. BALANCED 7:5 GRID (BIO & ARCHITECTURAL DOSSIER) ================= */}
      <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-cream/10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 flex flex-col justify-center space-y-4 sm:space-y-6">
            ...
            <div className="self-start inline-flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-ochre/10 border border-ochre/30 text-ochre font-mono text-[10px] sm:text-[11px] tracking-wider uppercase leading-normal">
            ...
            <p className="font-sans text-sm sm:text-base md:text-[18px] text-[#A8A29A] leading-[1.75] sm:leading-[1.8] font-normal text-justify">
          ...
          <div className="lg:col-span-5 relative overflow-hidden group p-5 sm:p-6 rounded-lg bg-[#100E0C] border border-cream/15 transition-colors hover:border-cream/25 flex flex-col justify-between">
            {/* Dossier Header */}
            ...
            <div className="divide-y divide-cream/5">
              <div className="flex items-center justify-between gap-3 py-2.5 border-b border-cream/5 group/row hover:bg-cream/[0.02] hover:px-2 transition-all duration-300">
                <span className="font-mono text-[11px] text-cream/40 uppercase tracking-wider shrink-0">Primary Focus</span>
                <span className="font-mono text-[11px] text-cream/90 font-medium text-right">Software Engineering</span>
              </div>
              <div className="flex items-center justify-between gap-3 py-2.5 border-b border-cream/5 group/row hover:bg-cream/[0.02] hover:px-2 transition-all duration-300">
                <span className="font-mono text-[11px] text-cream/40 uppercase tracking-wider shrink-0">Systems Depth</span>
                <span className="font-mono text-[11px] text-cream/90 font-medium text-right">Full-Stack &amp; Research</span>
              </div>
              <div className="flex items-center justify-between gap-3 py-2.5 border-b border-cream/5 group/row hover:bg-cream/[0.02] hover:px-2 transition-all duration-300">
                <span className="font-mono text-[11px] text-cream/40 uppercase tracking-wider shrink-0">Timezone</span>
                <span className="font-mono text-[11px] text-cream/90 font-medium text-right">WIB (UTC+7) // Global Ready</span>
              </div>
              <div className="flex items-center justify-between gap-3 py-2.5 border-b border-cream/5 group/row hover:bg-cream/[0.02] hover:px-2 transition-all duration-300">
                <span className="font-mono text-[11px] text-cream/40 uppercase tracking-wider shrink-0">Data Integrity</span>
